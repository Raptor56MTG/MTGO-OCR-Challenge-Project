...
import cv2
import difflib
from .config import IMAGE_THRESH, IMAGE_PATH, IMAGE_RESIZED, IMAGE_GRAY, IMAGE_DISPLAYED
from .utils import get_best_match_score, get_best_match_username


def preprocess_image() -> object:
    """This uses preprocessing techniques tuned for mtgo images
    to improve accuracy. These files are then saved locally
    for later access and use. These techniques are listed below.
    - resize image.
    - grayscale image (used for error correction)
    - binary image with thresholding (used for error correction)

    Returns
    -------
    binarized black-white image to be used in error correction."""
    ...
    cv2.imwrite(IMAGE_THRESH, im_bw)
    return im_bw


def run_easyocr() -> list:
    """This reads the uploaded image using easyocr and returns
    the easyocr generated list of tuples.

    Returns
    -------
    full list of tuples generated by easyOCR"""

    im_bw = preprocess_image()
    reader = easyocr.Reader(['en'])
    results = reader.readtext(IMAGE_RESIZED)
    return sort_easyocr(results, im_bw)


def sort_easyocr(results: list, im_bw: object) -> list:
    """This function handles the corner case where easyOCR
    ...
    Parameters
    ----------
    results : full list of tuples generated by easyOCR
    im_bw: the binarized image of the original inputted image.
...
def correct_easyocr(line: list) -> str:
    """This corrects common easyOCR errors and returns it in
    the corrected csv format.
    ...
    Parameters
    ----------
    line : list containing text easyOCR generated.

    Returns
    -------
    String in csv format (example below)
...
def generate_csv(results: list) -> None:
    """Generate a csv file from the results that easyOCR generated.

    Parameters
    ----------
    results : List of text easyOCR generated.

    Returns
    -------
    Does not return any values. Writes to a file named output.csv."""
...
def display_output(results: list) -> None:
    """Display what easyocr found for quick proof reading for the user.
    ...
    Returns
    -------
    Does not return any values. Creates and writes an image-display.png file."""
    ...
    cv2.imwrite(IMAGE_DISPLAYED, img)
